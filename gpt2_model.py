...
class GPT2Config:
    ...
    model_name: str = "gpt2-small"             # TransformerLens model name
    activation_name: str = "post"         # Activation hook name to extract
    layer_num: int = 0                   # Layer number (0-11 for gpt2-small)
    batch_size: int = 128                # Batch size for processing
    context_length: int = 128            # Context length for training data
    device: str = "cuda" if torch.cuda.is_available() else "cpu"
    dtype: torch.dtype = torch.float16   # Data type for model (fp16 for memory efficiency)
    buffer_size: int = 10000             # Number of tokens to keep in buffer
    dataset_name: str = "NeelNanda/pile-10k"  # HF Dataset to load
    seed: int = 42                       # Random seed


class GPT2Model:
    """
    GPT-2 model wrapper from TransformerLens.
    ...
    def __init__(self, config: GPT2Config):
        ...
        print(f"Initialized GPT-2 model with {self.n_layers} layers and {self.d_mlp} MLP dimension")
    ...
    def _load_dataset(self):
        ...
        try:
            ...
            data = load_dataset(self.config.dataset_name, split="train[:1%]")
